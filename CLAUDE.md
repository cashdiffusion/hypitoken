...
### Market (校园集市) — `internal/market`

A second independent storefront, sibling of `shop`, on its own listener (`:8320`, `endpoints.market` + `market.enabled`). Models **physical campus goods sold for a deposit** rather than card secrets: a buyer pays a configurable **deposit fraction** (`deposit_ratio`, default 0.10) of the price through **Z-Pay** (`billing.ZPayGateway`, Alipay/WeChat), which **reserves one unit** — a product shows "SOLD OUT" once `quantity` units are reserved (`pending`+`paid`+`fulfilled` orders; a pending order frees its unit when the 30-min sweeper expires it). Orders carry the buyer's **fulfilment choice** (自提 at `pickup_location` / 配送到楼 needing a dorm address) + contact. **Reuses shop.db** (own `market_*` tables + `market_schema_version` track — two pools on one WAL file). Mobile-first server-rendered Go `html/template` (reuses shop's oklch theme), camera-capture photo upload to a local disk dir (`image_dir`, served at `/img/:product_id/:file`). Admin reuses `cfg.AdminToken` cookie. Wired via `AttachExtraEndpoint` in `cmd/server/main.go` exactly like shop; descriptions are **manual, no AI**. Order/product data rides shop.db so the daily backup covers it; **`market_images/` on disk is NOT backed up**.
