...
//     (bare — no cache_control, matches real 2.1.167).
...
// shape of a real Claude Code 2.1.167 CLI request. Returns the original body
...
func applyClaudeCodeBodyMimicry(body []byte, model string, id SimIdentity) []byte {
	...
	// Step 1: rebuild system to match the CC 2.1.167 4-block layout.
	...
	// NOTE: real CC 2.1.167 also carries `thinking` ({"type":"adaptive"}),
	...
	//
	// `diagnostics` (new at CC 2.1.167) is semantically neutral —
...
// rewriteSystemForOAuth rebuilds the system field to match the real CC 2.1.167
...
// Mirrors the real CC 2.1.167 capture exactly.
func applySystemCacheBreakpoints(blocks []json.RawMessage) {
	if len(blocks) == 0 {
		return
	}
	// Real CC 2.1.167 puts scope:global on the SECOND-TO-LAST system block
...
// last block of the last message — exactly what real CC 2.1.167 does
...
// 2.1.167 capture, so we don't add it.
